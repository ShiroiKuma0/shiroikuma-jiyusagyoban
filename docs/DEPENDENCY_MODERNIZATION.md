...
| Built-in Kotlin / AGP new DSL | enabled |
...
| Room | 2.8.4 |
| Room 3 | deferred; only 3.0.0-alpha03 available |
| WorkManager | 2.11.2 |
| DataStore | 1.2.1 |
| Coroutines | 1.10.2 |
| Hilt / Dagger | 2.59.2 |
...
2. Room with exported-schema review and migration-test execution. Completed on the existing `androidx.room` 2.x line with `2.8.4` after Kotlin/KSP alignment. Room 3.0 uses the new `androidx.room3:room3-*` artifact group and is deferred because official Maven metadata currently exposes only `3.0.0-alpha03`.
...
9. Built-in Kotlin and AGP new DSL migration. Completed by removing the explicit `org.jetbrains.kotlin.android` plugin alias, dropping `android.builtInKotlin=false` and `android.newDsl=false`, and replacing the deprecated `androidTest` assets `srcDirs` call with the AGP 9 `directories` set.
10. Room 3 artifact-group migration. Deferred until a stable Room 3 release exists because this is a data-risk, package/group-changing migration across runtime builders, annotations, KSP compiler artifacts, migration helpers, schema export, and instrumentation migration tests.
...
- 2026-05-05: Migrated the AGP 9 build to built-in Kotlin and the new DSL by removing `org.jetbrains.kotlin.android`, deleting the temporary AGP 9 opt-out flags, and replacing the deprecated `androidTest` assets source-set mutation. Verified AAR metadata, debug Kotlin/unit tests, debug androidTest compile, debug APK/lint, F-Droid release profile with metadata checks, and install/start smoke on `SM-S938B - 16` with `AutomationService` foreground. Remaining dependency modernization is the separate Room 3 artifact-group migration decision.
- 2026-05-05: Reviewed the Room 3 artifact-group migration. Official Google Maven metadata exposes `androidx.room3:room3-runtime`, `room3-ktx`, and `room3-compiler` only at `3.0.0-alpha03`, with no stable Room 3 release. Decision: keep Room `2.8.4` and defer Room 3 until stable because the migration changes package/import surface and touches both persisted databases plus instrumentation migration tests.
