...
2. Room with exported-schema review and migration-test execution. Completed on the existing `androidx.room` 2.x line with `2.7.2`; Room `2.8.x` needs the later Kotlin/KSP/compiler batch, and Room 3.0 uses a new artifact group that remains a separate migration decision.
...
5. AndroidX Core KTX, Coroutines, DataStore, Kotlinx Serialization, and Gson in small runtime-focused batches. Completed the API 35 / Kotlin 2.0-compatible subset with Core KTX `1.16.0`, DataStore `1.2.1`, and Gson `2.14.0`; Core KTX `1.17.x+` requires API 36 / AGP 8.9.1, and Coroutines `1.10.x+` plus Kotlinx Serialization `1.8.x+` require newer Kotlin metadata than the current Kotlin `2.0.21` compiler reads.
...
- 2026-05-05: Upgraded Room from `2.6.1` to `2.7.2` on the existing `androidx.room` artifact line. A trial upgrade to `2.8.4` failed during KSP schema export with a `kotlinx.serialization` generated-serializer ABI mismatch, and the later runtime-support pass confirmed current Kotlin `2.0.21` cannot read the newer Kotlin metadata required by Kotlinx Serialization `1.8.x+`, so Room `2.8.x` is deferred until the Kotlin/KSP/compiler batch. Room 3.0 stays out of this batch because it uses the new `androidx.room3` group. Verified debug Kotlin/unit tests, debug androidTest compile, debug APK/lint, F-Droid release profile, and `:app:connectedDebugAndroidTest` on `SM-S938B - 16` with the two migration tests passing.
