...
9. Built-in Kotlin and AGP new DSL migration. Completed by removing the explicit `org.jetbrains.kotlin.android` plugin alias, dropping `android.builtInKotlin=false` and `android.newDsl=false`, and replacing the deprecated `androidTest` assets `srcDirs` call with the AGP 9 `directories` set. Room 3 remains a separate artifact-group migration decision.
