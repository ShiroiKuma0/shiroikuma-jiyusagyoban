...
- The L7 template expression baseline now has a pure bounded `{{ ... }}` engine wired into action argument expansion after legacy `%var` expansion, with task/event/global scope precedence, arrays, JSON path reads, string/math functions, traces, sanitized expanded-argument summaries, and fail-closed warnings. Remaining L7 work should expose debugger UI around traces, decide regex policy separately, adopt templates in conditions if needed, and render richer per-step expansion details in run-log details.
