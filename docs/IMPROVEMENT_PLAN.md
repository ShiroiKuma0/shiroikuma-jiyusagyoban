...
- Day schedule polish now uses a shared parser for aliases, numeric tokens, and ranges, saves canonical day lists from the UI, blocks invalid day schedules before save, and shows readable day summaries in profiles and Context Inspector. Remaining context polish should keep using shared parsers instead of raw text-only config wherever a small bounded grammar exists.
- X10 now includes a Tasker XML document-picker flow, bounded preview UI, mapped-action and unsupported-action reporting, explicit confirmation, and Room import through the OpenTasker bundle repository with profiles disabled by default.
...
- The L6 FOSS geofence baseline now centralizes Haversine radius checks, optional accuracy gating, and dwell-time evaluation in `FossGeofenceEvaluator`, wires active Location contexts through it, registers a platform `LocationManager` source with fail-closed setup events and provider-specific balanced cadence defaults, starts the foreground automation service from app launch and boot, verifies on an API 36 device that app launch starts the service foreground with `specialUse|location` after location prerequisites are granted/enabled, adds `tools/collect-location-evidence.ps1` for repeatable foreground-service, location, logcat, structured battery, provider-cadence, Room, and run-log snapshots, adds a setup-required disabled Location evidence template, verifies background Location event delivery for that template on an API 36 device with Room run-log evidence after the app was sent home, adds fail-closed `-RequireUnpluggedSample`, `-RequireProviderCadenceEvidence`, and post-reconnect `-RequireRecentUnpluggedHistory` harness gates, records a 615.055-second API 36 unplugged-history sample with provider cadence present, shares Android 11+/14+ location policy disclosure copy across Setup and Context Inspector, persists inside-since state per profile/context/config before matching, surfaces dwell state in Context Inspector profile rows, and clears stale profile-scoped dwell keys when profiles are deleted or their context lists change. Remaining L6 work should repeat provider/device verification before public geofence claims.
