...
- The L7 template expression baseline now has a pure bounded `{{ ... }}` engine wired into action argument expansion after legacy `%var` expansion, with task/event/global scope precedence, arrays, JSON path reads, string/math functions, sanitized expanded-argument summaries, warning counts, and persisted per-expression run-log diagnostics. Remaining L7 work should decide regex policy separately and adopt templates in conditions if needed.
