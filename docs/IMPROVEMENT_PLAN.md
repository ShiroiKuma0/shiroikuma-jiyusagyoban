...
- The L7 template expression baseline now has a pure bounded `{{ ... }}` engine with task/event/global scope precedence, arrays, JSON path reads, string/math functions, traces, and fail-closed warnings. Remaining L7 work should wire the engine into action argument expansion, expose debugger UI around traces, decide regex policy separately, and show sanitized expanded arguments in step-level run logs.
