...
- The L6 FOSS geofence baseline now centralizes Haversine radius checks, optional accuracy gating, and dwell-time evaluation in `FossGeofenceEvaluator`, wires active Location contexts through it, registers a platform `LocationManager` source with fail-closed setup events and provider-specific balanced cadence defaults, persists inside-since state per profile/context/config before matching, surfaces dwell state in Context Inspector profile rows, and clears stale profile-scoped dwell keys when profiles are deleted or their context lists change. Remaining L6 work should add device-backed battery measurement, expanded policy disclosures, and device-level background-location verification before public geofence claims.
