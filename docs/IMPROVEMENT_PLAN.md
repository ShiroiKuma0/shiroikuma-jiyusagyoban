...
- The L6 FOSS geofence baseline now centralizes Haversine radius checks, optional accuracy gating, and dwell-time evaluation in `FossGeofenceEvaluator`, wires active Location contexts through it, registers a platform `LocationManager` source with fail-closed setup events and provider-specific balanced cadence defaults, starts the foreground automation service from app launch and boot, verifies on an API 36 device that app launch starts the service foreground with `specialUse|location` after location prerequisites are granted/enabled, adds `tools/collect-location-evidence.ps1` for repeatable foreground-service, location, logcat, battery, Room, and run-log snapshots, adds a setup-required disabled Location evidence template for later event-delivery smoke tests, shares Android 11+/14+ location policy disclosure copy across Setup and Context Inspector, persists inside-since state per profile/context/config before matching, surfaces dwell state in Context Inspector profile rows, and clears stale profile-scoped dwell keys when profiles are deleted or their context lists change. Remaining L6 work should install/enable that template on a device, verify actual background Location event delivery with the new assertions, and run longer unplugged battery samples before public geofence claims.
