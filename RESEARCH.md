...
- Accessibility gap is narrowed but remains: audited icon-only `contentDescription = null` cases are gone; action checkbox rows, widget task rows, and flow graph targets now expose better roles/state. A full TalkBack sweep and accessibility-checks instrumentation gate are still missing.
