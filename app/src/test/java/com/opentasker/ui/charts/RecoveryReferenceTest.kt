...
class RecoveryReferenceTest {
    ...
    fun `the NSF recommended window is the top step`() {
        assertEquals(5, RecoveryReference.sleepStep(h(7.0)))
        assertEquals(5, RecoveryReference.sleepStep(h(8.0)))
        assertEquals(5, RecoveryReference.sleepStep(h(9.0)))
    }
    ...
    fun `falling short ranks below overshooting by the same amount`() {
        ...
        assertEquals(4, RecoveryReference.sleepStep(h(9.5)))
    ...
    fun `the boundaries are exactly where the consensus puts them`() {
        assertEquals("7h is recommended, not below it", 5, RecoveryReference.sleepStep(h(7.0)))
        assertEquals("just under 7h is not", 3, RecoveryReference.sleepStep(h(7.0) - 1))
        assertEquals("9h is still recommended", 5, RecoveryReference.sleepStep(h(9.0)))
        assertEquals("just over 9h is not", 4, RecoveryReference.sleepStep(h(9.0) + 1))
        assertEquals("6h enters may-be-appropriate", 3, RecoveryReference.sleepStep(h(6.0)))
        assertEquals("just under 6h leaves it", 2, RecoveryReference.sleepStep(h(6.0) - 1))
        assertEquals("10h is the top of may-be-appropriate", 4, RecoveryReference.sleepStep(h(10.0)))
        assertEquals("just over 10h leaves it", 2, RecoveryReference.sleepStep(h(10.0) + 1))
    }

    @Test
    fun `the extremes are the bottom step on both sides`() {
        assertEquals(1, RecoveryReference.sleepStep(h(4.0)))
        assertEquals(1, RecoveryReference.sleepStep(h(12.0)))
        assertEquals(1, RecoveryReference.sleepStep(0.0))
    }
    ...
    fun `the recorded nights land where they should`() {
        ...
        assertEquals("5h51 is below may-be-appropriate", 2, RecoveryReference.sleepStep(351.0))
        assertEquals("7h40 meets it", 5, RecoveryReference.sleepStep(460.0))
        assertEquals("8h32 meets it", 5, RecoveryReference.sleepStep(512.0))
        assertEquals("9h18 overshoots it", 4, RecoveryReference.sleepStep(558.0))
    }
    ...
    fun `the resting-rate decades map one step each`() {
        assertEquals(5, RecoveryReference.nocturnalHrStep(48.0))
        assertEquals(4, RecoveryReference.nocturnalHrStep(55.0))
        assertEquals(3, RecoveryReference.nocturnalHrStep(64.0))
        assertEquals(2, RecoveryReference.nocturnalHrStep(75.0))
        assertEquals(1, RecoveryReference.nocturnalHrStep(88.0))
    }

    @Test
    fun `each decade boundary belongs to the better side below it`() {
        assertEquals(5, RecoveryReference.nocturnalHrStep(49.9))
        assertEquals(4, RecoveryReference.nocturnalHrStep(50.0))
        assertEquals(4, RecoveryReference.nocturnalHrStep(59.9))
        assertEquals(3, RecoveryReference.nocturnalHrStep(60.0))
        assertEquals(3, RecoveryReference.nocturnalHrStep(69.9))
        assertEquals(2, RecoveryReference.nocturnalHrStep(70.0))
        assertEquals(2, RecoveryReference.nocturnalHrStep(79.9))
        assertEquals(1, RecoveryReference.nocturnalHrStep(80.0))
    }
    ...
    fun `a higher heart rate never scores better`() {
        ...
        for (bpm in 40..110) {
            ...
            assertTrue("$bpm scored above the rate below it", step <= previous)
            previous = step
        }
    }
}
