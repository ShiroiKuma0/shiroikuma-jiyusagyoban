...
class LocalizationSourceTest {
    ...
    fun presentationSurfacesUseStringResourcesForVisibleCopy() {
        val localizedFiles = listOf(
            "com/opentasker/ui/components/PremiumComponents.kt",
    ...
    fun dynamicActionAndContextCatalogsUseCompleteResourceIds() {
        ...
        assertEquals("Expected every built-in action name to be resource backed", 67, Regex("""nameRes = R\.string\.catalog_action_""").findAll(metadata).count())
        assertEquals("Expected every action field to be resource backed", 151, Regex("""ActionField\(\s*\"""").findAll(metadata).count())
