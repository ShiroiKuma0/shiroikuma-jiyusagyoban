...
class LocalizationSourceTest {
    ...
    fun dynamicActionAndContextCatalogsUseCompleteResourceIds() {
        ...
        assertEquals("Expected every built-in action name to be resource backed", 80, Regex("""nameRes = R\.string\.catalog_action_""").findAll(metadata).count())
        assertEquals("Expected every action field to be resource backed", 201, Regex("""ActionField\(\s*\"""").findAll(metadata).count())
