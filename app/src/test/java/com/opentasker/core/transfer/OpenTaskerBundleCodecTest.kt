...
class OpenTaskerBundleCodecTest {
    ...
    // REMOVED: upstream's jsonExportRedactsALiteralCopyOfASecretValue. It exercises
    // OpenTaskerBundleCodec.sanitizeForExport, which this fork does not have — the fork's
    // serializer writes the name-based BundleFile DTO straight out and runs no redaction pass
    // over action arguments. Restore the test with the function if that ever changes.
