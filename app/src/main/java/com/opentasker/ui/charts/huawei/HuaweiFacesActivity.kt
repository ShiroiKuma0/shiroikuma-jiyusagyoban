            ...
            OpenTaskerTheme(prefs = themePrefs) {
                CompositionLocalProvider(
                    ...
                    Surface(Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.background) {
                        ...
                        // An AtomicBoolean rather than Compose state: the poll runs on
                        // HuaweiSyncRunner's scope and reads this from another thread every 100 ms,
                        // which is neither a recomposition nor something to make one.
                        val identifyStop = remember { java.util.concurrent.atomic.AtomicBoolean(false) }
                        ...
                        androidx.compose.runtime.LaunchedEffect(dir) {
                            ...
                            state = state.copy(
                                faces = faces,
                                loading = false,
                                names = HuaweiSettings.faceNames(applicationContext),
                            )
                        }
                        ...
                        HuaweiFacesScreen(
                            ...
                            onIdentifyStart = {
                                if (state.bandBusy || state.identify != null) return@HuaweiFacesScreen
                                identifyStop.set(false)
                                state = state.copy(identify = IdentifyState(running = true))
                                HuaweiSyncRunner.scope.launch {
                                    // The stop flag is read from the state, so pressing Done ends
                                    // the poll AND closes the session — a held connection that
                                    // outlived its dialog would be the standing-link problem again.
                                    val r = HuaweiSyncRunner.watchShowingFace(
                                        applicationContext,
                                        HuaweiSettings.address(applicationContext),
                                        shouldStop = { identifyStop.get() },
                                        onShowing = { store ->
                                            scope.launch {
                                                val cur = store?.faces?.firstOrNull { it.showing }
                                                state = state.copy(
                                                    // The whole list is refreshed too: the band is
                                                    // being handled, and what it holds may change
                                                    // under us while this is open.
                                                    band = store ?: state.band,
                                                    identify = state.identify?.copy(
                                                        running = true, showing = cur,
                                                    ),
                                                )
                                            }
                                        },
                            ...
                            onIdentifyStop = {
                                identifyStop.set(true)
                                state = state.copy(identify = null)
                            },
                            onAskRemoveBandFace = { face ->
                                state = state.copy(confirmRemove = face)
                            },
                            onRemoveBandFace = { face ->
                                if (state.bandBusy) return@HuaweiFacesScreen
                                state = state.copy(deleting = face.assetId, message = null)
                                HuaweiSyncRunner.scope.launch {
                                    val label = state.names[face.assetId] ?: face.assetId
                                    ...
                                    // The band is the authority on what it kept, here as in the
                                    // grid: a delete that quietly failed must not leave the list
                                    ...
                                    scope.launch {
                                        state = state.copy(
                                            ...
                                            message = when {
                                                r.getOrNull() == true -> "$label — removed"
                                                else -> "$label — ${r.exceptionOrNull()?.message ?: "still on the band"}"
                                            },
                                        )
                                    }
                                }
                            },
                            onNameFace = { assetId, name ->
                                HuaweiSettings.setFaceName(applicationContext, assetId, name)
                                state = state.copy(
                                    names = HuaweiSettings.faceNames(applicationContext),
                                )
                            },
