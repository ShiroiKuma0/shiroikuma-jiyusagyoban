...
object ActionCapabilityRegistry {
    ...
    private val capabilities = mapOf(
        ...
        // Upstream's new "write an Android setting by name" action. RequiresSetup, not Unsupported:
        // unlike the elevated actions this one has a route a user can actually take without root or
        // Shizuku — a single ADB grant that then persists.
        ...
        // Upstream's lock action now works: a force-lock-only device admin, activated from Setup.
        // The fork's old "Unsupported" claim would refuse to add an action that runs.
        "lock" to ActionCapability(CapabilityLevel.RequiresSetup, "Requires the Lock screen device admin, activated from Setup.", CapabilityRequirement.DeviceAdmin),
        ...
        "bubble.freeze_add" to ActionCapability(CapabilityLevel.RequiresSetup, "The freeze bubbles render only with \"display over other apps\" and only while the Desktop launcher is foreground. The bubble is queued either way — it is the drawing that waits.", CapabilityRequirement.Overlay),
        ...
        "huawei.time" to ActionCapability(CapabilityLevel.RequiresSetup, "Moves the paired HUAWEI band's own clock — needs the Nearby devices (Bluetooth) permission. For looking at a watch face as it renders at another hour; the next connection restores the phone's time by itself.", CapabilityRequirement.Bluetooth, blocking = true),
        ...
        "huawei.almanacwatch" to ActionCapability(CapabilityLevel.Supported, "Asks the Galileo almanac's publisher, every half hour in the background, whether something newer than the file a set was built on has appeared — and posts a notification when it has. It reads one small public file per check, touches no device, and stops by itself after a day."),
        "huawei.pgnss" to ActionCapability(CapabilityLevel.Supported, "Builds the band's 72-hour predicted-ephemeris set on the phone from free public orbit products, and writes it into the folder the GNSS action serves from. It downloads about 25 MB every run — there is deliberately no cache, because a kept orbit product is a wrong one a day later — and then spends several minutes of every core on the fit. It touches no device and reads nothing personal."),
