...
fun registerActionMetadata() {
    ...
    ActionMetadataRegistry.register(
        ActionMetadata(
            ...
            fields = listOf(
                ...
                ActionField("remove", "Remove only", FieldType.TEXT, hint = "<assetId>_<version> to delete from the band, installing nothing. Making room is its own job — everything else here needs a face to send"),
                ActionField("evict", "Remove first", FieldType.TEXT, hint = "<assetId>_<version> to delete from the band before installing, in the same session. Needed when the band is full, and when reinstalling a face it already holds — otherwise the install is short-circuited to an activate and no bytes move"),
                ActionField("address", "Band address", FieldType.TEXT, hint = "blank = the band's known address"),
    ...
    ActionMetadataRegistry.register(
        ActionMetadata(
            id = "huawei.pgnss",
            name = "Huawei Band predicted ephemeris",
    ...
    ActionMetadataRegistry.register(
        ActionMetadata(
            ...
            fields = listOf(
                ...
                ActionField("prefix", "Variable prefix", FieldType.TEXT, hint = "default HUAWEI_"),
                ActionField("store", "Store the result in", FieldType.TEXT, hint = "a variable name to receive the one-line result"),
                ActionField("raw", "Replay these exact bytes", FieldType.TEXT, hint = "diagnostic. Hex for the whole 0x0F/0x01 payload, sent verbatim instead of the fields above — the way to test Health's captured frame without guessing which of five differences matters"),
                ActionField("enable", "Send the weather-on first", FieldType.CHECKBOX, hint = "diagnostic. 0x0F/0x0C {129:{2:01}} — the ON half of the switch, which Health sends at pairing and we never send at all"),
                ActionField("uv", "UV index", hint = "0–11. Shown on the band's own UV page"),
                ActionField("wind", "Wind speed (km/h)", hint = "the band works the Beaufort level out from this itself"),
                ActionField("hourly_start", "First hourly entry (epoch seconds)", hint = "from the source's own alignment. Defaults to flooring the clock to the hour, which is wrong in a half-hour time zone"),
                ActionField("hourly_uv", "Hourly UV index", hint = "comma-separated, same hours as `hourly`. The band's \"Highest level\" is the maximum of these. An EMPTY element is a gap and keeps its position — the hour is filled by carrying the last real reading"),
                ActionField("hourly_condition", "Hourly conditions", hint = "comma-separated words (clear, partly_cloudy, rain, snow, …), same hours as `hourly`"),
                ActionField("hourly_feels", "Hourly apparent temperature (°C)", hint = "comma-separated, same hours as `hourly`. Defaults to the plain temperature"),
                ActionField("daily_high", "Daily highs (°C)", hint = "comma-separated, FIRST = today. The band refuses a forecast with fewer than 8 days, so a shorter list is stretched by repeating the last one"),
                ActionField("daily_low", "Daily lows (°C)", hint = "comma-separated, same days as `daily_high`"),
                ActionField("daily_condition", "Daily conditions", hint = "comma-separated words, same days as `daily_high`"),
                ActionField("hourly", "Hourly temperatures (°C)", hint = "comma-separated, FIRST = the hour we are in. The band refuses a forecast that is not exactly 24 hours, so a shorter list is stretched by repeating the last value — which is a flat line, not a forecast. Give it 24 when the source has them"),
                ActionField("daily", "Days", hint = "one per ';', each high/low or high/low/condition, FIRST = today"),
                ActionField("pre_reads", "Health's three reads first", FieldType.CHECKBOX, hint = "diagnostic. Sends 0x0F/0x02, 0x06 and 0x0A ahead of the push, where Health sends them. The forecast has never once been applied by the band; this is the remaining difference in the sequence"),
                ActionField("read_back", "Read the band back afterwards", FieldType.CHECKBOX, hint = "diagnostic. Asks 0x0F/0x02, 0x06 and 0x0A with tag 1 zero-length (the READ form) and reports the answers, in the hope of an oracle that does not need eyes on the band"),
            ),
        ),
    )
    ...
    ActionMetadataRegistry.register(
        ActionMetadata(
            ...
            fields = listOf(
                ...
                ActionField("warmup", "Warm-up before listening", FieldType.TEXT, hint = "diagnostic. Comma-separated from 0107,0105,0102,0103,0137,1a05 — or `all`. These are the six commands Huawei Health sends on EVERY reconnect and our routine session sends none of; the band's first unsolicited frame arrives 104 ms after the last of them. None writes a setting, a clock or any wizard state"),
    ...
    ActionMetadataRegistry.register(
        ActionMetadata(
            id = "settings.write",
            name = "Write Setting",
            description = "Change one Android setting by name. Global and Secure need one-time access granted from a computer; System uses Modify system settings.",
            category = "Settings",
            fields = listOf(
                ActionField(
                    "table",
                    ...
                    FieldType.DROPDOWN,
                    required = true,
                    hint = "Where the setting lives. Most are Secure or Global.",
                    options = listOf("global", "secure", "system"),
                ),
                ActionField(
                    "key",
                    "Setting name",
                    FieldType.TEXT,
                    required = true,
                    hint = "location_mode, screen_off_timeout, etc.",
                    sensitive = false,
                ),
                ActionField(
                    "value",
                    ...
                    FieldType.TEXT,
                    required = true,
                    hint = "Read back after writing, so a value Android ignores reports a failure.",
                ),
            )
        )
    )
