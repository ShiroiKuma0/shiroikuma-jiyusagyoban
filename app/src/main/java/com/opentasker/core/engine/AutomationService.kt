...
class AutomationService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (intent?.action == ACTION_RUN_EXTERNAL_TASK) {
            ...
            if (executionId != null && taskId != null) {
                scope.launch {
                    ensureEngineLoaded()
                    runExternalTask(executionId, taskId, variables, runSource)
                }
            }
            return START_STICKY
        }
    ...
    /** Set once the matchers are loaded, so a cold-process run reloads them exactly once. */
    ...
    private fun runAutoStartTasks() {
        scope.launch {
            ...
            AutoStartSettings.migrate(this@AutomationService, db)
            for (name in AutoStartSettings.taskNames(this@AutomationService)) {
                val task = db.taskDao().getByNameIgnoreCase(name)?.toDomain()
                if (task == null) {
                    ...
                    AppLogger.info(
                        "OpenTasker",
                        "Auto-start: no task named “$name” — it was removed or renamed. " +
                            "Re-add it under Monitor → Run on start.",
                    )
                    continue
                }
