...
class TaskRunner(
    ...
    private suspend fun runSubTask(
        ...
        // nested run starts, waits behind the active one, or is skipped outright. The nested run goes
        // through its own Job (see runNestedUnderItsOwnJob) so a collision on the sub-task can no
        // longer cancel the caller that invoked it.
        val coordinator = collisionCoordinator
        val report = if (coordinator == null) {
            child.run(target)
        } else {
            when (val collision = runNestedUnderItsOwnJob(coordinator, target, child)) {
...
private fun actionTimeoutMs(actionType: String): Long = when {
    ...
    // Building the predicted set is the longest piece of arithmetic this app does, and it starts
    // with about six minutes of downloading before any of it. See the constant.
    actionType == "huawei.pgnss" -> HUAWEI_PGNSS_TIMEOUT_MS
...
/**
 * Building the 72-hour predicted-ephemeris set: `HuaweiPgnssAction.MAX_RUN_MS` plus margin.
 *
 * The download was measured at 368 s for 21.3 MB on 2026-08-30 — 299 s of it the BeiDou orbits over
 * Wuhan's own FTP at about 18 KB/s — and is now a small fraction of that, since those files come
 * from IGN's mirror at roughly two orders of magnitude more. The budget is NOT reduced to match:
 * what follows is 2196 fifteen-parameter least-squares fits plus a
 * numerical integration of every BeiDou satellite across three days. Measured on a desktop that is
 * roughly three minutes of CPU across all cores; on the phone, at a third of the per-core speed and
 * on cores-minus-two, it is several times that.
 *
 * The action caps ITSELF at 55 minutes so that the message the panel shows is its own and not this
 * budget killing it where it stands. This sits just above that: it is the backstop, not the plan.
 */
private const val HUAWEI_PGNSS_TIMEOUT_MS = 3_360_000L // 55 min cap + 1 min margin
