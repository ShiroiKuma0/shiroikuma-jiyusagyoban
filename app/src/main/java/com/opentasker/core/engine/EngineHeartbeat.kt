...
object EngineHeartbeat {
    ...
    /** A profile matcher's flow errored and was auto-restarted (self-heal — clock/charging recovery). */
    fun markMatcherRestart(name: String, now: Long = System.currentTimeMillis()) {
        add(HeartbeatEvent(now, HeartbeatEvent.Kind.MATCHER_RESTART, 0L, name))
    }
    ...
    data class HeartbeatEvent(val atMs: Long, val kind: Kind, val staleMs: Long, val label: String = "") {
        enum class Kind { STARTED, REARMED, RESURRECTED, MATCHER_RESTART }
    }
}
