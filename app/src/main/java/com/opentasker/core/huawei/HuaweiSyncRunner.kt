...
object HuaweiSyncRunner {
    ...
    /**
     * Set the band's own clock.
     *
     * Every session already pushes the phone's time in `announceCompanion`, so this must be sent
     * AFTER that — which it is, being the body of the session rather than part of opening it. The
     * same fact makes it self-healing: the next connection to the band puts the real time back
     * whether or not anyone asks it to.
     */
    suspend fun setBandTime(
        context: Context,
        address: String,
        epochSeconds: Long,
    ): Result<String> = withSession(context, address) { session, _ ->
        val zone = java.util.TimeZone.getDefault()
        val offset = zone.getOffset(epochSeconds * 1000) / 60_000
        session.request(
            HuaweiCommands.SVC_DEVICE_CONFIG, HuaweiCommands.CMD_SET_TIME,
            HuaweiCommands.setTime(epochSeconds, offset / 60, offset % 60),
            timeoutMs = 6_000,
        )
    ...
    /**
     * Watch which face the band is showing, while 白い熊 cycles through them ON the band.
     *
     * ## Why a held session and not repeated calls
     *
     * [listWatchFaces] opens a session, asks once and closes. Doing that every two seconds would
     * spend the whole time connecting, and a band being reconnected to twice a minute is exactly
     * the standing-link problem in miniature. So this opens ONE session and asks inside it.
     *
     * ## Why this exists at all
     *
     * The band lists nine of 白い熊's faces as bare ten-digit ids, because Huawei Health installed
     * them and this library has no copy. When the band is full, the picker has to ask which one may
     * go — and answering that from a number alone risks deleting a stock face that cannot be put
     * back (白い熊, 2026-08-30: "I don't want to delete a stock one I like and end up without it").
     * The band already says which face is on screen, so the answer is to read it while he turns the
     * dial: each face announces its own id, and it can then be named.
     *
     * [onShowing] is called with the currently-showing face on every poll, or null when the band
     * reports none. It is called on every poll rather than only on change, so the caller can tell a
     * live watch from a stalled one.
     */
    suspend fun watchShowingFace(
        context: Context,
        address: String,
        pollMs: Long = 2_000,
        maxMs: Long = 10 * 60_000,
        shouldStop: () -> Boolean,
        onShowing: (HuaweiUploadClient.FaceStore?) -> Unit,
    ): Result<Unit> = withSession(context, address) { session, _ ->
        val client = HuaweiUploadClient(session)
        val deadline = System.currentTimeMillis() + maxMs
        while (!shouldStop() && System.currentTimeMillis() < deadline) {
            onShowing(client.listWatchFaces())
            // Sliced, so stopping is felt at once rather than at the end of a poll.
            var slept = 0L
            while (slept < pollMs && !shouldStop()) {
     ...
     */
    /**
     * Install a face, optionally giving one up to make room.
     *
     * ## The removal is its OWN session, and that is not tidiness
     *
     * Deleting and installing inside one connection does not work: the band accepts the delete,
     * reports the face gone and the slot free, and then refuses the install with **`wf/3
     * r=100007`** — measured on 2026-08-31 at eleven faces with the band's own figure saying
     * eighty-four free, and repeatedly. The same install into the same slot succeeds immediately
     * afterwards over a fresh connection. Whatever the band is doing, it does not consider the
     * removal to have taken effect until the session that performed it has ended.
     *
     * That refusal is indistinguishable from a genuinely full band — both are 100007 — which is
     * exactly why the evict-in-session bug survived: it always looked like the condition it was
    ...
    suspend fun uploadWatchFace(
        ...
        onProgress: (Int) -> Unit = {},
    ): Result<HuaweiUploadClient.Outcome> {
        val name = file.name.removeSuffix(".bin")
        val assetId = name.substringBefore('_')
        val version = name.substringAfter('_', "")
        if (version.isEmpty()) {
            return Result.failure(IllegalArgumentException("$name is not <assetId>_<version>"))
        }
        ...
        if (!meta.isFile) {
            return Result.failure(
                IllegalArgumentException("missing ${meta.name} beside the face — capture it with the file"),
            )
        }
        ...
        if (evict != null) {
            ...
            val removed = withSession(context, address) { session, _ ->
                HuaweiUploadClient(session).deleteWatchFace(evict.first, evict.second)
            }
            if (removed.getOrNull() != true) {
                return Result.success(
                    HuaweiUploadClient.Outcome(
        ...
        return withSession(context, address) { session, _ ->
    ...
    suspend fun serveGnss(
        ...
        /**
         * Every file finished so far, with the running byte total.
         *
         * A completed transfer IS a result, and until this existed the panel had nowhere to put one
         * until the whole action returned — so a finished file sat under a RESULT box still reading
         * "Transferring…". The log's "complete" was true; the box simply had nothing to report yet
         * (白い熊, 2026-08-30).
         */
        onServed: (served: List<String>, bytes: Int) -> Unit = { _, _ -> },
        /**
        ...
        // Whether the PREDICTED set is on the table at all, and when the band last said anything.
        // Both feed the decision below about when a quiet line means "over" rather than "waiting".
        val predicted = files.keys.filter { it.startsWith(PGNSS_PREFIX) }
        var lastHeard = System.currentTimeMillis()

        /**
         * Is there anything left worth waiting for?
         *
         * The predicted set being complete IS the update, and the moment it is complete there is
         * nothing else to hold the link open for: the broadcast file is a separate errand the band
         * runs whenever it wants a fix, and waiting on it left the panel spinning on step 4 with
         * Bluetooth up while the band's own screen already said complete (白い熊, 2026-08-30).
         */
        fun done(): Boolean =
            if (predicted.isNotEmpty()) served.containsAll(predicted) else served.size >= files.size
        ...
        loop@ while (!cancelled && System.currentTimeMillis() < deadline) {
            ...
            if (f == null) {
                // A QUIET LINE IS NOT THE END OF THE SESSION.
                //
                // This used to break out of the loop the first time fifteen seconds passed without
                // a frame, and that threw the whole feature away: the band asks for the BROADCAST
                // file on its own whenever it wants a fix, we served that 7 KB in ten seconds, went
                // quiet, and closed Bluetooth — while 白い熊 was still walking to the band to press
                // Update. `wait` was 600 seconds and we used 10 of them (白い熊, 2026-08-30: "it
                // didn't send the data to the band, there was no update").
                //
                // The caller's deadline is the only thing that ends the watch, with two exceptions:
                // everything offered has been taken, or the predicted round has plainly happened
                // and then gone quiet. Before a single predicted file has moved there is nothing to
                // settle and we wait the whole time we were given.
                if (done()) break@loop
                // Cancel, polled BETWEEN rounds only. Mid-transfer it stays ignored on purpose —
                // a half-written file the band believes is whole is worse than a slow exit.
                if (shouldCancel()) {
                    cancelled = true
                    onProgress("Cancelled", "Stopped between rounds")
                    log.append("cancelled between rounds; ")
                    break@loop
                }
                val predictedStarted = predicted.isEmpty() || served.any { it in predicted }
                if (predictedStarted && System.currentTimeMillis() - lastHeard > SETTLE_MS) break@loop
                continue@loop
            }
            ...
            when (f.commandId) {
                ...
                HuaweiCommands.GNSS_DONE -> {
                    ...
                    current?.let {
                        served.add(it)
                        onProgress(
                            if (done()) "Done" else "Waiting for the band to ask again",
                            "$it complete",
                        )
                        onServed(served.toList(), bytes)
                    }
                    current = null
                    if (done()) break@loop
                }
     ...
     */
    /** The predicted set's file names all start with this; the broadcast one does not. */
    const val PGNSS_PREFIX = "HW_PGNSS_"

    /**
     * How long the band may be silent AFTER the predicted round before the watch calls it over.
     *
     * Only applied once a predicted file has actually moved. Before that, silence means the band
     * has not been told to update yet, and the caller's own deadline is what governs.
     */
