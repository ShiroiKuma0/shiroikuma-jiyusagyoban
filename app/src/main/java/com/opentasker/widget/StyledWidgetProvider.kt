    ...
    companion object {
        ...
        fun refreshAll(context: Context): Int {
            ...
            val entries = ids.map { id ->
                renderWidget(context, manager, id)
                val name = StyledWidgetStore.getName(context, id)
                val template = StyledWidgetStore.getTemplate(context, id)
                val label = when {
                    name.isNotBlank() && template != null -> "$name ← $template"
                    name.isNotBlank() -> name
                    template != null -> template
                    else -> "#$id"
                }
                // isPull = template-bound → re-expands globals every tick; a static layout never updates.
                com.opentasker.core.engine.WidgetRefreshLog.Entry(label, template != null)
            }
            com.opentasker.core.engine.WidgetRefreshLog.record(entries)
            return ids.size
        }
