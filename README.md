...
**📥 Latest release: [`0.2.93+2026-09-14.00-34.gaa1a372a+148`](https://github.com/ShiroiKuma0/shiroikuma-jiyusagyoban/releases/latest)** — [all releases & APK downloads »](https://github.com/ShiroiKuma0/shiroikuma-jiyusagyoban/releases)
...
The reason this fork exists. A generic action that fires arbitrary **explicit or implicit intents** (action, data URI, MIME type, string/int/bool extras, target component, flags) — including the automation intents exposed by the sister apps (a token is optional there since v2, and one sent to an app that does not ask for it is ignored) (e.g. `白い熊 GNU Jami` / `shiroikuma.jami`: send-message, place-call, open-conversation; `白い熊 音楽` / `shiroikuma.ongaku`: favorite-track, delete-track, play-playlist). `<queries>` manifest entries make the explicit targets resolve on Android 11+.
...
**45 watch faces captured and installable — and one written from scratch.** The captured ones are lifted from Health's own downloads, verified against the digest the phone sent the band, and archived one ZIP per face with its preview. A grid on the phone installs any of them, reads back what the band is holding, and removes one. That last part cost a face: `0x27/0x03` tag 3 is **01 = install, 02 = DELETE**, and there is no activate command at all — the version that "activated" a face was deleting the one it had just sent.

**The band does not check the signature**, and that was measured rather than assumed: a face with one byte flipped deep inside it installs exactly as its pristine control does, digest be damned. So faces can be *made*. **相撲字時計** is ours — a clock in **勘亭流**, the brush lettering of sumo banners, painted glyph by glyph into a donor face's slots, whose minute line reads **丁度** on the hour and **半** at half past. Getting those two readings needed the line rebuilt: a slot picks the image matching its value and a tile covers what is under it, so the row is a decision list, and a centred reading makes 丁度 an exclusive-or of the two digits — which no decision list computes. 「バンドの時計」 stands the band's clock at any date and minute, so a face is looked at without waiting an hour for it.
...
**It serves the band its own satellite data, with no Huawei account anywhere.** `huawei.gnss` is the one place where the band drives and the phone answers: it raises a request, names what it wants, and pulls over a second service in an order we do not choose. Broadcast ephemeris comes from an open Huawei endpoint — no token, no signature — is unpacked from gzip and served on demand; asked while an Outdoor Walk was starting, the band took our own 7403-byte RTCM file 39 seconds after the watch opened. The band asks for **two different things**: its Update button wants the *predicted* set, signed with a credential Huawei issues at runtime to Health's own package and certificate; when it actually needs a fix it asks for broadcast ephemeris instead, which can be refreshed freely and forever. Huawei's predicted endpoint is therefore closed to us — so `scripts/pgnss-build.py` **builds the predicted set instead**, fitting CODE's free five-day orbit prediction to Kepler element sets against the band's own propagator and writing the EEV2 layout byte-for-byte. Verified against the precise orbit at **GPS median 0.03 m, Galileo 0.03 m**. With a set alive the band fixes in about **8 s**; forty-one minutes after one expired, with fresh broadcast ephemeris served the moment it asked, the same walk took **582 s** — and on a set that had quietly gone four days stale, **1135 s**, which is why a wholly expired set is now refused rather than handed over: the band takes one, marks its data current, and stops asking for the broadcast file it could still use. The build **reads its own output back** before calling itself done, records `NOT REBUILT — <reason>` on any run that did not really rebuild, and raises a magenta banner on the panel whenever nothing reached the band — because the failure that cost those four days was a build that threw on a header line while the panel still read “done”. **And the file that actually decides the fix is the almanac, not the ephemeris.** The predicted set graded at 0.11 m while the band took two minutes to find itself: the almanac says *which* satellites to look for and roughly where, and the ephemeris only matters once one has been found. Ours was carrying Huawei's captured BeiDou records forward without the Earth-rotation term — `ωe·604800 = 44.1027 rad` is not a multiple of 2π, so it folds to **6.904° per week** of gap to a packaged capture, and five satellites walked east a little further every Sunday until the geostationaries were 27.6° off station, which is 20 000 km. Corrected, the same walk fixed in **8 s** on a set generated end to end, with only QZSS still Huawei's. The geostationaries are now checked against their **published station longitudes** — no orbit product, no fixture, no network — and a build that puts one off its station is **refused rather than written**, because a set the band accepts and then searches the wrong sky with is worse than none. **And what decays across a window is the CLOCK, not the orbit.** Graded against CODE's own final observed orbits over a whole 72-hour window, our shipped orbits go from 0.3–0.4 m at the start to **1.0–1.5 m at the end** — metre-class for three days, for the three constellations CODE supplies. The clocks over the same span go from 1–9 m to **5–27 m median**, with Galileo's 95th percentile at 132 m on day three, growing monotonically in every six-hour bucket the way a straight line's error does. That is not ours to fit away: the clock is a line through the product's own clock, so the error is CODE's prediction error, and nothing free predicts clocks better across three days. It is bounded by rebuilding, not by better arithmetic — which is also why the orbit-model work this project kept circling was dropped: it was aimed at a decay the measurement says is not there. **And a satellite that will not encode is dropped rather than fatal**: `NaN > 50` is false, so a non-finite fit once walked through every bound and killed the build in the encoder, three times in one morning. **And the last thing to learn was that none of that matters if the band never receives it.** A set built on 2026-09-18 was perfect — every almanac in it published that same day — and the transfer after it moved **zero bytes**, so the band went on wearing a set whose window had closed the previous afternoon and fixed the slow way for four days while the phone reported itself in perfect health. Every record this feature kept was about the phone: what it built, what it offered. What the band *accepted*, and until when that forecast is good, was written nowhere. Now `served-log.txt` keeps one line per transfer — a failure as loudly as a success — `band-holds.txt` remembers the window the band actually took, and the panel warns about **the band's** forecast rather than the phone's. Run the same transfer again with the same bytes and the band asks within 6 seconds and fixes in **8**. **And the phone now says how long that forecast is good for, out loud, in days and hours** — the moment the band accepts a set, with two exact alarms behind it, twelve hours before the end and at it; tapping the notification opens the whole thing full-screen. The band's own countdown is **not** our window: it counts 72 hours from the moment it *took* the set, measured to the minute, so it over-claims by exactly the delay between building a set and handing one over — under two hours in a single run, a whole day when a set built on Thursday reaches the band on Friday. Both numbers are printed side by side with the reason they differ. And **a build that fails no longer passes for a fresh set**: the run that lost four days died on a DNS lookup, walked on to the transfer anyway, handed the band the previous set and announced success. The serve refuses while a build failure is standing, never overwrites that failure with silence, and takes a freshness bar from the caller — a run that has just built a set holds 72 hours, so materially less means the build did not happen. **And the almanac is measured in kilometres now, not dated in days.** An age was always a proxy for the thing that matters, and on 2026-09-25 the proxy failed: a build shipped a Galileo almanac published three days earlier whose worst satellite sat **2 227 km** from where that same set's own ephemeris put it, against 96 km for a current one — the band searched the wrong sky and there was no fix before the walk, with everything needed to say so already on the phone and nothing measuring it. Each Keplerian almanac is now propagated with **the receiver's own broadcast formula** and differenced against **CODE's precise orbit** at the start, middle and end of the window, scored on the worst; a healthy two-day-old almanac measures **2 km** for GPS and **52 km** for Galileo, so the scale separates cleanly from the failure. The distance sits beside the age on the panel, in the summary, and in **every `built-log.txt` line** — the line actually read after a bad walk — and a *current* almanac that places badly raises its own alert, which the age gate would have waved through. GLONASS keeps the bare age rather than borrowing a number: its almanac is not Keplerian, and a figure produced by pretending otherwise would be worse than none. **And when the publisher has simply not published yet, the phone keeps looking.** A daily source and a build at half past seven is a race nobody should have to run by hand, so the panel offers — only while the almanac is actually stale — a background search that checks every 30 minutes for up to 24 hours and says the moment something newer appears, with the rebuild one tap from the news. It does not rebuild by itself: a build is ten minutes of radio and then wants 白い熊 at the band. Its first name, 「新しい概略暦を待つ」, read as sit-and-do-nothing and was the opposite of what it does, so every string it owns names the cadence and its ongoing notification carries a **stop button** — a search that can only be called off from a panel three taps away is one nobody can call off.
...
Thirteen columns now, in the order the night happened — date, how you woke, night HR,
**going-to-bed HR**, **the night's HR swing**, **the night's curve**, asleep, **deep**,
**deep+REM**, **lowest HR**, **HRV (RMSSD)**, **breathing** and **SpO₂** — in one horizontal scroll,
every cell one line, sized against the device's real font scale rather than the default. Temperature
is gone: this band has none, so the column was a dash on every line.

**One of those columns draws instead of printing.** A swing in bpm is not
heart-rate-through-the-night: 14 bpm that fell early and 14 bpm that rose at four in the morning are
the same number and different nights, which is why the report draws a curve rather than printing the
swing. So the table carries the curve itself, one small line per row, scored by its **descent** —
how far that night's fall came short of the usual one — and deliberately **not** by the swing beside
it, because a swing cannot tell a fall from a rise. The first version was coloured by the swing and
drew a night that never fell in ordinary blue while the report's own bar for the same night was dark
red; two numbers for one quantity is the failure this project trips over most, and the two screens
...
**Freeze App** and **Unfreeze App** apply and lift the strongest lock the phone has. Where [白い熊 雫](https://github.com/ShiroiKuma0/shiroikuma-shizuku) is the Device Owner and delegates package access, a freeze is a real **device-policy suspension** — filed under the owner's admin, so no shell command can undo it — and needs no Shizuku at all; otherwise it falls back to `pm disable-user`, and the run log names the gates that landed. A freeze applies the same **four gates** 白い熊 応用管理 does — force-stop, the policy suspension, `pm disable-user`, and the policy **hide** — so an app re-frozen from a bubble is exactly as held as it was found. Unfreeze clears all four, **unhiding first**: a hidden package reads as *not installed* to every lookup, which is what a thaw has to fix before it can do anything else. Success is reported only from a fresh read of the package, never from an exit code, and a defrost that fails names the missing power — the delegation, or Shizuku — rather than saying a lock is still held. `shiroikuma.shizuku`, `shiroikuma.oyokanri` and this app itself are refused outright. The **Make Launcher Tasks** action pops a **near-fullscreen multi-select grid of app-icon tiles** (all installed user apps, *including frozen ones*) — each tile shows the **package id under a bold label**, search matches **name or id**, and a ⚙ panel makes icon size, both text sizes, bold, and grid padding **settable and persistent** — and, on OK, writes one **unfreeze-then-launch** task per chosen app into a project group — re-sorted alphabetically on every run, with no duplicates. Each generated task takes the app's own icon, and whether it drops a re-freeze bubble is the roster's call, not the generator's. **An app is identified by its package, never by its label** — four of the Rovio games installed here answer to `Angry Birds`, so a name-based duplicate check skipped every one after the first and reported success having created nothing, with a UNIQUE (project, name) index waiting behind it. A name already taken in the project is qualified with the package instead, and the grid opens with every app that already has a task **pre-ticked and listed first** — a reading, not a switch: leaving a tick on creates nothing twice, and taking one off deletes nothing.

### 🫧 Freeze bubbles — re-freeze from the Desktop, on terms the workspace sets
A port of the Tasker 凍結 融解 idea that has since been turned inside out. Running a launcher task drops a small **draggable bubble**, shown **only while your home launcher (the Desktop) is in the foreground** — nowhere else, so nothing intrudes while you work. **Tap to freeze and dismiss, long-tap to just dismiss, ↗ to reopen.** Bubbles survive reboots, keep their position relative to the top-right edge across rotation and fold, and are styleable (icon size, roundness, label size/weight/font) with a live preview.

**What a bubble DOES is not in the app.** Each gesture resolves a **named workspace task** — `凍結泡 ⇨ 凍結`, `⇨ 捨てる`, `⇨ 起動` — run with the app's package, label and freeze set as per-invocation variables, and a per-app `<App> ⇦ 凍結` task overrides the first of them. The *existence* of that task is the whole override: nothing to enable, nothing to list, and it is readable in the workspace rather than compiled in. Each gesture keeps a built-in fallback, so a renamed task degrades instead of turning a bubble into a control that retires itself and does nothing.

**What a bubble freezes is read off the task that thawed it** — every package, not just the first. A launcher that thaws a banking app's companion before the app itself re-freezes both, and the pair can never drift from the launch behaviour it mirrors. **Which apps get a bubble** is an ordinary setting, `%Toketsu_Bubbles`, published by the project's 01 task and ticked in a picker that writes back to it. `bubble.freeze_add` / `_remove` / `_clear` let a task queue or retire one directly. The three apps that keep the automation running can never get a bubble, guarded in the single place every producer passes through.
...
The `音楽端灯` project pairs the workspace with the **白い熊 音楽** player (`shiroikuma.ongaku`, the Felicity fork) through a clean two-way contract: the player broadcasts **play-state and track changes** (title, artist, path, favorite) into `%INTENT_*` variables, and overlay **良 / 削除 buttons** — shown only while the player is foreground and playing — fire its **automation intents**: toggle-favorite, and a **confirmed delete** (song title + artist in the dialog) that lets the player itself skip, SAF-delete the file, and clean its library. Named **play-playlist tasks** (start playlist X, optionally at track Y) work from home-screen shortcuts. The audio-reactive **edge meteors that used to live here now render natively inside 白い熊 音楽** — beat-locked from the player's own decoder tap (sample-accurate, works under Android Auto and Bluetooth offload) — so this app no longer holds an audio `Visualizer` at all, and playback audio offload is never blocked.
