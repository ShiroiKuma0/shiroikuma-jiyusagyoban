...
**Status:** Baseline completed in v0.2.24. Added a pure automation graph model and optional read-only Flow tab that renders profile context nodes, enter/exit task lanes, action step nodes, edges, and incomplete-reference warnings. The current unreleased L1 work adds typed node targets, selectable Flow nodes that open the existing profile/context/task/action editors, missing-task repair routing to the owning profile, and first-class conditional action edge labels/node markers. Editing, branching authoring, future flow-control branch visualization, and canvas interactions remain later L1 slices.
