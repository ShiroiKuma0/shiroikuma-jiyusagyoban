...
**Status:** Baseline completed in v0.2.23. Plugin and library versions are centralized in `gradle/libs.versions.toml`, Gradle declarations use catalog aliases, and `docs/DEPENDENCY_MODERNIZATION.md` defines the staged upgrade order and verification gate. Follow-up batches upgraded Hilt/Dagger from `2.46` to intermediate `2.52`, Room from `2.6.1` to `2.7.2` on the existing `androidx.room` line, WorkManager from `2.9.1` to `2.11.2`, and the compatible Compose/AndroidX UI set to Compose BOM `2025.07.00` and Activity Compose `1.10.1` while leaving Kotlin/KSP/AGP and runtime startup unchanged; Room `2.8.x` is deferred to the Kotlinx Serialization/runtime-support batch after a KSP schema-export ABI mismatch, Compose BOM `2025.08.01+`, Hilt Navigation Compose `1.3.0`, and Lifecycle `2.9.x+` are deferred until the AGP/Kotlin lint stack is newer, and newer Activity/Navigation lines are deferred until the API 36 / AGP 8.9+ batch. Remaining version bumps are runtime support libraries, Kotlin/KSP/compiler alignment, and AGP.
